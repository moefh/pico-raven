...
void wii_i2c_decode_classic(const uint8_t *data, struct wii_i2c_classic_state *state)
{
    ...
    state->d_lt = (data[5] & 0x80) ? 0 : 1;
    state->d_rt = (data[5] & 0x04) ? 0 : 1;
    ...
    state->zl    = (data[4] & 0x20) ? 0 : 1;
    state->zr    = (data[4] & 0x02) ? 0 : 1;
}
