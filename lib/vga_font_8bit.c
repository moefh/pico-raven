        ...
        if (ch >= FIRST_CHAR && ch < FIRST_CHAR+NUM_CHARS) {
            ...
            for (int i = 0; i < font->h; i++) {
                ...
                for (int j = 0; j < font->w; j++) {
                    if ((char_line & char_bit) != 0 && y+i < vga_screen.height && x+j < vga_screen.width) {
                        vga_screen.lines8[y+i][x+j] = color;
                    }
                    char_bit <<= 1;
                }
            }
