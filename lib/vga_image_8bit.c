...
static void draw_image_line_mask0(uint32_t *screen, const uint32_t *image, int image_width, uint32_t colors)
{
    for (int x = 0; x < image_width/4; x++) {
        unsigned int mask = GET_4PIX_TRANSP_MASK(*image);
        if (mask == 0xffffffff) {
            ...
            *screen = (*screen & ~mask) | (colors & mask);
            screen++;
            image++;
        }
    }
    ...
    unsigned int mask = GET_4PIX_TRANSP_MASK(*image);
    switch (image_width%4) {
    case 1: *screen = (*screen & ((~mask) | 0xffffff00)) | (colors & mask); break;
    case 2: *screen = (*screen & ((~mask) | 0xffff0000)) | (colors & mask); break;
    case 3: *screen = (*screen & ((~mask) | 0xff000000)) | (colors & mask); break;
    }
}

static void draw_image_line_mask1(uint32_t *screen, const uint32_t *image, int image_width, uint32_t colors, bool skip_first_block)
{
    unsigned int cur, old;
...
static void draw_image_line_mask2(uint32_t *screen, const uint32_t *image, int image_width, uint32_t colors, bool skip_first_block)
{
    unsigned int cur, old;
...
static void draw_image_line_mask3(uint32_t *screen, const uint32_t *image, int image_width, uint32_t colors, bool skip_first_block)
{
    unsigned int cur, old;
...
static void draw_image(const struct VGA_IMAGE *img, const uint32_t *data, int img_x, int img_y, bool transparent)
{
    const uint32_t *img_start = data;

    int height = img->height;
    if (img_y < 0) {
        img_start += img->stride * (-img_y);
        height += img_y;
        img_y = 0;
    }
    if (height > vga_screen.height - img_y) height = vga_screen.height - img_y;
    if (height <= 0) return;
    ...
    if (width > vga_screen.width - img_x) width = vga_screen.width - img_x;
    if (width <= 0) return;

    const uint32_t *s = img_start;
    int s_stride = img->stride;
    uint32_t *d = vga_screen.lines[img_y] + img_x/4;
    int d_stride = vga_screen.width / 4;
    if (transparent) {
        switch (img_x % 4) {
        case 0: for (int y = 0; y < height; y++) { draw_image_line_tr0(d, s, width); d += d_stride; s += s_stride; } break;
        case 1: for (int y = 0; y < height; y++) { draw_image_line_tr1(d, s, width, skip_first); d += d_stride; s += s_stride; } break;
        case 2: for (int y = 0; y < height; y++) { draw_image_line_tr2(d, s, width, skip_first); d += d_stride; s += s_stride; } break;
        case 3: for (int y = 0; y < height; y++) { draw_image_line_tr3(d, s, width, skip_first); d += d_stride; s += s_stride; } break;
        }
    } else {
        switch (img_x % 4) {
...
static void draw_image_mask(const struct VGA_IMAGE *img, const uint32_t *data, int img_x, int img_y, uint32_t colors)
{
    const uint32_t *img_start = data;
    ...
    int d_stride = vga_screen.width / 4;
    switch (img_x % 4) {
    case 0: for (int y = 0; y < height; y++) { draw_image_line_mask0(d, s, width, colors); d += d_stride; s += s_stride; } break;
    case 1: for (int y = 0; y < height; y++) { draw_image_line_mask1(d, s, width, colors, skip_first); d += d_stride; s += s_stride; } break;
    case 2: for (int y = 0; y < height; y++) { draw_image_line_mask2(d, s, width, colors, skip_first); d += d_stride; s += s_stride; } break;
    case 3: for (int y = 0; y < height; y++) { draw_image_line_mask3(d, s, width, colors, skip_first); d += d_stride; s += s_stride; } break;
    }
}

void vga_image_draw_frame_mask(const struct VGA_IMAGE *img, int frame, int x, int y, uint8_t color)
{
    uint32_t colors = (color << 24) | (color << 16) | (color << 8) | color;
    draw_image_mask(img, &img->data[frame*img->height*img->stride], x, y, colors);
}
...
void vga_image_draw(const struct VGA_IMAGE *img, int x, int y, bool transparent)
{
