...
static uint8_t sound_bits_per_sample;
...
static void __isr __time_critical_func(dma_handler)(void)
{
  ...
  // ack dma irq
  dma_hw->ints1 = 1u << sound_dma_chan;
}

int sound_i2s_init(const struct sound_i2s_config *cfg)
{
  sound_bits_per_sample = cfg->bits_per_sample;

  // allocate sound buffers
  size_t sound_buffer_size = (sound_bits_per_sample/8) * 2 * SOUND_I2S_BUFFER_NUM_SAMPLES;
  ...
  // setup pio
  sound_pio = (cfg->pio_num == 0) ? pio0 : pio1;
  uint offset = pio_add_program(sound_pio, (sound_bits_per_sample == 8) ? &sound_i2s_8bits_program : &sound_i2s_16bits_program);
  sound_pio_sm = pio_claim_unused_sm(sound_pio, true);
  if (sound_bits_per_sample == 8) {
    sound_i2s_8bits_program_init(sound_pio, sound_pio_sm, offset, cfg->sample_rate, cfg->pin_data, cfg->pin_clock_base);
  } else {
    sound_i2s_16bits_program_init(sound_pio, sound_pio_sm, offset, cfg->sample_rate, cfg->pin_data, cfg->pin_clock_base);
  }
...
void sound_i2s_playback_start(void)
{
  ...
  channel_config_set_transfer_data_size(&dma_cfg, (sound_bits_per_sample == 8) ? DMA_SIZE_16 : DMA_SIZE_32);
