...
- sound: I2S with 16-bits using a MAX98357 amplifier
- controller: I2C using a Wii Classic controller
...
The physical connections are shown here:
...
Sound effects are just sample buffers in flash or RAM, and are mixed directly. The system supports 4 simultaneous channels, but this can easily be changed at compile-time; currently a conservative 4 is chosen to avoid problems but it will probably be increased as the project develops.

The MOD music source is generated by a custom simple MOD player that supports old-school 4-channel MOD files with the a single upgrade: samples are allowed to be either 8-bits or 16-bits.

## Controller

We use Wii Classic controllers, as they're simple to use (they speak I2C) and cheap clones are available.

I found that the easiest and most reliable way to connect to a Wii Classic controller is to get the female socket from an extension cord, which is available for cheap online.
...
Above: console front showing raven head (*wow*) and the controller and sound plugs hanging

![Console case - Back](console-back.jpg)  
Above: console back showing VGA connector and hole for the Pico 2 USB slot (for power or firmware update)
